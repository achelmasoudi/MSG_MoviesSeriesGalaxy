...
class AdapterOfComments(var activity: Activity, var commentsList: ArrayList<ModelOfComments>) :
    RecyclerView.Adapter<AdapterOfComments.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfComments: ModelOfComments = commentsList.get(position)

        holder.username.text = modelOfComments.username // or holder.username.setText(modelOfComments.username)
        holder.comment.text = modelOfComments.comment
        holder.dateOfComment.text = modelOfComments.dateOfComment

        //Glide for downloaing the profile picture from Fb
        Glide.with(activity).load(modelOfComments.profilePic)
        ...
        name = modelOfComments.name

        var commentId = modelOfComments.commentId
        var profilePic = modelOfComments.profilePic
        var username = modelOfComments.username
        var comment = modelOfComments.comment
        var dataOfComment = modelOfComments.dateOfComment
        ...
        holder.reactionButton.setOnClickListener {
            if (holder.likeReactionIcon.tag == "selectedLike") {
                ...
                removeReactionFromFirebase(commentId, "Likes")
            }
            else if (holder.loveReactionIcon.tag == "selectedLove") {
                // If the reaction is already selected (Love), remove it
                removeReactionFromFirebase(commentId, "Loves")
            }
            else if (holder.laughReactionIcon.tag == "selectedLaugh") {
                // If the reaction is already selected (Laugh), remove it
                removeReactionFromFirebase(commentId, "Laughs")
            }
            else if (holder.angryReactionIcon.tag == "selectedAngry") {
                // If the reaction is already selected (Angry), remove it
                removeReactionFromFirebase(commentId, "Angry")
            }
            else {
                // If no reaction is selected, add Like
                addLike_Love_Laugh_AngryToFirebase(commentId, "Likes")
            }
            // Call isLike and isLove and isLaugh and isAngry to update the state after the action
            isLike(name, commentId, holder.likeReactionIcon, holder.likeReactionName , holder.defaultLikeReactionName)
            isLove(name, commentId, holder.loveReactionIcon, holder.loveReactionName, holder.defaultLikeReactionName)
            isLaugh(name, commentId, holder.laughReactionIcon, holder.laughReactionName, holder.defaultLikeReactionName)
            isAngry(name, commentId, holder.angryReactionIcon, holder.angryReactionName, holder.defaultLikeReactionName)
        }

        holder.reactionButton.setOnLongClickListener(object: View.OnLongClickListener{
            override fun onLongClick(v: View?): Boolean {
                if (currentUser != null) {
                    showDialogOfReactions(activity ,commentId , holder.likeReactionIcon , holder.defaultLikeReactionName ,
        ...
        updateNumberOfReactions(commentId, holder.numberOfReactions , icon1, icon2, icon3, icon4 , holder.seeAllReactionsBtn)

        // About the replies Of Comment
        holder.replyButton.setOnClickListener {
            aboutReplyProcess( name , username , profilePic , comment , commentId , dataOfComment)
        }

        // Put Count of replies for each comment
        val replyReference: DatabaseReference = FirebaseDatabase.getInstance().reference
            .child("CommentReplies").child(name).child(commentId)
    ...
    inner class MyViewHolder(iv: View) : RecyclerView.ViewHolder(iv) {
        ...
        init {
            ...
            commentBtn.setOnClickListener {
                ...
                var commentId = modelOfComments.commentId
                var userId = modelOfComments.userId
                var comment = modelOfComments.comment
                var profilePic = modelOfComments.profilePic
                var username = modelOfComments.username

                if (currentUser != null && modelOfComments.userId.equals(currentUser.uid)) {
                    showDialogOfDeleteEdit(activity,commentId, userId , comment , profilePic , username)
                }
                else {
                    // the comments is not of the current user
                    showDialogOfCopyReport_Reactions(activity,commentId, userId , comment , profilePic , username)
                }
            }
            ...
            profilePicImageView.setOnClickListener {
                var position: Int = adapterPosition
                var modelOfComments: ModelOfComments = commentsList.get(position)
                showDialogOfProfilePic(activity , modelOfComments.profilePic)
            }
        }
    }

    private fun aboutReplyProcess(name: String , username: String, profilePic: String, comment: String, commentId: String, dataOfComment: String) {
    ...
    private fun showDialogOfReactions(activity: Activity , commentId: String , reactionIcon: ImageView , defaultLikeReactionName: TextView ,
        ...
        // Like reaction
        likeReactionProcess(popupView , popupWindow, commentId , reactionIcon , likeReactionName , defaultLikeReactionName)
        // Love reaction
        loveReactionProcess(popupView , popupWindow , commentId , reactionIcon , loveReactionName , defaultLikeReactionName)
        // Laugh reaction
        laughReactionProcess(popupView , popupWindow , commentId , reactionIcon , laughReactionName , defaultLikeReactionName)
        // Angry reaction
        angryReactionProcess(popupView , popupWindow , commentId , reactionIcon , angryReactionName , defaultLikeReactionName)

    }
    private fun likeReactionProcess(popupView: View , popupWindow: PopupWindow , commentId: String , reactionIcon: ImageView , likeReactionName: TextView , defaultLikeReactionName: TextView) {
        ...
        likeReaction!!.setOnClickListener {
            ...
            isLike(name, commentId, reactionIcon, likeReactionName , defaultLikeReactionName)
        }
    }
    private fun loveReactionProcess(popupView: View , popupWindow: PopupWindow , commentId: String , reactionIcon: ImageView , loveReactionName: TextView , defaultLikeReactionName: TextView) {
        ...
        loveReaction!!.setOnClickListener {
            ...
            if (reactionIcon.tag.equals("unselectedLove")) {
                // Love the comment
                addLike_Love_Laugh_AngryToFirebase(commentId, "Loves")
            ...
            isLove(name, commentId, reactionIcon, loveReactionName , defaultLikeReactionName)

        }
    }
    private fun laughReactionProcess(popupView: View , popupWindow: PopupWindow , commentId: String , reactionIcon: ImageView , laughReactionName: TextView , defaultLikeReactionName: TextView) {
        ...
        laughReaction!!.setOnClickListener {
            ...
            if (reactionIcon.tag.equals("unselectedLaugh")) {
                ...
                addLike_Love_Laugh_AngryToFirebase(commentId, "Laughs")
            ...
            isLaugh(name, commentId, reactionIcon, laughReactionName , defaultLikeReactionName)

        }
    }

    private fun angryReactionProcess(popupView: View , popupWindow: PopupWindow , commentId: String , reactionIcon: ImageView , angryReactionName: TextView , defaultLikeReactionName: TextView) {
        ...
        angryReaction!!.setOnClickListener {
            ...
            if (reactionIcon.tag.equals("unselectedAngry")) {
                ...
                addLike_Love_Laugh_AngryToFirebase(commentId, "Angry")
            ...
            isAngry(name, commentId, reactionIcon, angryReactionName , defaultLikeReactionName)
        }
    }

    private fun addLike_Love_Laugh_AngryToFirebase(commentId: String, reactionType: String) {
        ...
        userReference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name).child(commentId).child(reactionType).child(currentUser.uid).setValue(data)
                }
            }
            override fun onCancelled(error: DatabaseError) {}
        })
    ...
    private fun removeReactionFromFirebase(commentId: String, reactionType: String) {
        // Remove the reaction from the specified type (Likes or Loves or Laughs or Angry)
        FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name).child(commentId).child(reactionType).child(currentUser.uid).removeValue()
    }

    private fun isLike(name: String, commentId: String, likeReactionIcon: ImageView, likeReactionName: TextView , defaultLikeReactionName: TextView) {
        var reference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name)
            .child(commentId).child("Likes")
    ...
    private fun isLove(name: String, commentId: String, loveReactionIcon: ImageView, loveReactionName: TextView , defaultLikeReactionName: TextView) {
        var reference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name)
            .child(commentId).child("Loves")
    ...
    private fun isLaugh(name: String, commentId: String, laughReactionIcon: ImageView, laughReactionName: TextView , defaultLikeReactionName: TextView) {
        var reference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name)
            .child(commentId).child("Laughs")
    ...
    private fun isAngry(name: String, commentId: String, angryReactionIcon: ImageView, angryReactionName: TextView , defaultLikeReactionName: TextView) {
        var reference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("CommentsReactions").child(name)
            .child(commentId).child("Angry")
    ...
    private fun updateNumberOfReactions(commentId: String, numberOfReactions: TextView , icon1: ImageView , icon2: ImageView , icon3: ImageView , icon4: ImageView , seeAllReactionsBtn: CardView) {
        val likesReference: DatabaseReference = FirebaseDatabase.getInstance().reference
            .child("CommentsReactions").child(name).child(commentId).child("Likes")
        val lovesReference: DatabaseReference = FirebaseDatabase.getInstance().reference
            .child("CommentsReactions").child(name).child(commentId).child("Loves")
        val laughsReference: DatabaseReference = FirebaseDatabase.getInstance().reference
            .child("CommentsReactions").child(name).child(commentId).child("Laughs")
        val angryReference: DatabaseReference = FirebaseDatabase.getInstance().reference
            .child("CommentsReactions").child(name).child(commentId).child("Angry")
                                    ...
                                    override fun onDataChange(angrySnapshot: DataSnapshot) {
                                        ...
                                        if (totalCount == 0L) {
                                            ...
                                            seeAllReactionsBtn.setOnClickListener {
                                                seeAllReactionsProcess(commentId , totalCount , likesCount , lovesCount , laughsCount , angryCount)
                                            }
                                        }
                                    }
                                    override fun onCancelled(error: DatabaseError) {}
    ...
    private fun seeAllReactionsProcess(commentId: String, totalCount: Long, likesCount: Long, lovesCount: Long, laughsCount: Long, angryCount: Long) {
    ...
    private fun showDialogOfDeleteEdit(activity: Activity, commentId: String, userId: String , comment: String , profilePic: String , username: String) {
        ...
        deleteBtn!!.setOnClickListener {
            deleteDialog(userId , commentId , bottomSheetDialog)
        }
        ...
        editBtn!!.setOnClickListener {
            editDialog(userId , commentId , comment , profilePic , username , bottomSheetDialog)
        }
    }

    private fun deleteDialog(userId: String, commentId: String, bottomSheetDialog: BottomSheetDialog) {
            ...
            var alertDialog: AlertDialog.Builder = AlertDialog.Builder(activity)
                ...
                .setPositiveButton("DELETE", object: DialogInterface.OnClickListener {
                    override fun onClick(dialog: DialogInterface?, which: Int) {
                        deleteFromFB(commentId , bottomSheetDialog)
                    }
                })
    ...
    private fun deleteFromFB(commentId: String, bottomSheetDialog: BottomSheetDialog) {
        var userReference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("Users").child(firebaseAuth.uid!!)
        userReference.addListenerForSingleValueEvent(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    var ref: DatabaseReference = FirebaseDatabase.getInstance().getReference("Comments").child(name).child(firebaseAuth.uid!!)
                    ref.addListenerForSingleValueEvent(object: ValueEventListener{
                        override fun onDataChange(snapshot: DataSnapshot) {
                            for (commentSnapshot: DataSnapshot in snapshot.children) {
                                var commentKey: String = commentSnapshot.key!!
                                if (commentKey != null && commentKey.equals(commentId)) {
    ...
    private fun editDialog(userId: String, commentId: String , comment: String , profilePicP: String, usernameP: String , bottomSheetDialog: BottomSheetDialog) {
        if (currentUser != null && userId.equals(currentUser.uid)) {
            bottomSheetDialog.dismiss()
            ...
            saveBtn.setOnClickListener {
                ...
                if(newComment.isEmpty()) {
                    ...
                    editFromFB(commentId , newComment , dialog , saveBtn_saveTxt , saveBtn_loadingAnimation)
                }
            }
    ...
    private fun editFromFB(commentId: String , newComment: String , dialog: AlertDialog , saveBtn_saveTxt: TextView , saveBtn_loadingAnimation: LottieAnimationView) {
        var userReference: DatabaseReference = FirebaseDatabase.getInstance().reference.child("Users").child(firebaseAuth.uid!!)
        userReference.addListenerForSingleValueEvent(object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    ref.addListenerForSingleValueEvent(object: ValueEventListener{
                        override fun onDataChange(snapshot: DataSnapshot) {
                            for (commentSnapshot: DataSnapshot in snapshot.children) {
                                var commentKey: String = commentSnapshot.key!!
                                if (commentKey != null && commentKey.equals(commentId)) {
                                    if (commentSnapshot.hasChild("comment")) {
                                        // Update the oldComment to newComment
                                        commentSnapshot.child("comment").ref.setValue(newComment)
    ...
    private fun showDialogOfCopyReport_Reactions(activity: Activity, commentId: String, userId: String , comment: String , profilePic: String , username: String) {
        var bottomSheetView: View = LayoutInflater.from(activity).inflate(R.layout.bottom_sheet_of_delete_and_edit_comment,
            activity.findViewById(R.id.bottomSheetLayout_deleteComment_containerId))
        ...
        reportCommentBtn!!.setOnClickListener {
            reportCommentDialog(userId , commentId , comment , profilePic , username , bottomSheetDialog)
        }
    }
    ...
    private fun reportCommentDialog(userId: String, commentId: String, comment: String, profilePicP: String, usernameP: String, bottomSheetDialog: BottomSheetDialog) {
