...
class AdapterOfLoves(var context: Context, var list: ArrayList<ModelOfLoves>) : RecyclerView.Adapter<AdapterOfLoves.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfLoves: ModelOfLoves = list.get(position)
        holder.username.text = modelOfLoves.username
        holder.icon.setImageResource(R.drawable.love_comments_reaction)

        //Glide for downloaing the profile picture from Fb
        Glide.with(context).load(modelOfLoves.profilePic).listener(object :
