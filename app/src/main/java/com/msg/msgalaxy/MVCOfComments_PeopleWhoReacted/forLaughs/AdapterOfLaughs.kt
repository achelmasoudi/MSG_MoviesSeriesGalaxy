...
class AdapterOfLaughs(var context: Context, var list: ArrayList<ModelOfLaughs>) : RecyclerView.Adapter<AdapterOfLaughs.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfLaughs: ModelOfLaughs = list.get(position)
        holder.username.text = modelOfLaughs.username
        holder.icon.setImageResource(R.drawable.laugh_comments_reaction)

        //Glide for downloaing the profile picture from Fb
        Glide.with(context).load(modelOfLaughs.profilePic).listener(object :
