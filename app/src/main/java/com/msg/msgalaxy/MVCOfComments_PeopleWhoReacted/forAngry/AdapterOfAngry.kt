...
class AdapterOfAngry(var context: Context , var list: ArrayList<ModelOfAngry>): RecyclerView.Adapter<AdapterOfAngry.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfAngry: ModelOfAngry = list.get(position)
        holder.username.text = modelOfAngry.username
        holder.icon.setImageResource(R.drawable.angry_comments_reaction)

        //Glide for downloaing the profile picture from Fb
        Glide.with(context).load(modelOfAngry.profilePic).listener(object :
