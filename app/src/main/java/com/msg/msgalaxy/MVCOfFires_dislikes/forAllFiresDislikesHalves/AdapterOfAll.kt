...
class AdapterOfAll(var context: Context, var allList: ArrayList<ModelOfAll>) : RecyclerView.Adapter<AdapterOfAll.MyViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.fires_dislikes_card_item, parent, false)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfAll: ModelOfAll = allList.get(position)
        holder.username.text = modelOfAll.username

        Glide.with(context).load(modelOfAll.profilePic).listener(object: RequestListener<Drawable> {
        ...
        if (modelOfAll.iconId == 1) {
            holder.icon.setImageResource(R.drawable.selected_fire_icon)
        }
        else if (modelOfAll.iconId == 2) {
            holder.icon.setImageResource(R.drawable.selected_half_icon)
        }
        else holder.icon.setImageResource(R.drawable.selected_dislike_icon)
    }
    ...
    inner class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            profilePicImageView = itemView.findViewById(R.id.firesDislikesCardItem_profilePicId)
            username = itemView.findViewById(R.id.firesDislikesCardItem_usernameId)
            icon = itemView.findViewById(R.id.firesDislikesCardItem_firesOrDislikesIconId)
            lottieAnimationLoading = itemView.findViewById(R.id.firesDislikesCardItem_loadingLottie)
        ...
        private fun showPic() {
            ...
            var modelOfAll = allList.get(position)
            ...
            Glide.with(context).load(modelOfAll.profilePic).apply(RequestOptions.fitCenterTransform()).into(profilePicImageView)
        }
    }
}
