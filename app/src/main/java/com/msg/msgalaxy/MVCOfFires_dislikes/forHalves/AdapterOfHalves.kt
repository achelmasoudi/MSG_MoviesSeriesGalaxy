...
class AdapterOfHalves (var context: Context, var halvesList: ArrayList<ModelOfHalves>) : RecyclerView.Adapter<AdapterOfHalves.MyViewHolder>(){
    ...
    override fun getItemCount(): Int = halvesList.size

    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        var modelOfHalves: ModelOfHalves = halvesList.get(position)
        holder.username!!.text = modelOfHalves.username
        holder.halfIcon!!.setImageResource(R.drawable.selected_half_icon)

        //Glide for downloaing the profile picture from Fb
        Glide.with(context).load(modelOfHalves.profilePic).listener(object :
    ...
    inner class MyViewHolder(i: View) : RecyclerView.ViewHolder(i) {
        ...
        init {
            ...
            halfIcon = i.findViewById(R.id.firesDislikesCardItem_firesOrDislikesIconId)
        ...
        private fun showPic() {
            ...
            var modelOfHalves = halvesList.get(position)
            ...
            Glide.with(context).load(modelOfHalves.profilePic).apply(RequestOptions.fitCenterTransform()).into(profilePicImageView)
        }
    }
}